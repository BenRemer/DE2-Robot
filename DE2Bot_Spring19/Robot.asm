...
Init:
	...
	LOAD	Mask235		; Loads sonar 2 and 3 and 5 and 6 and 7
	OUT 	SONAREN	
...
currState:  DW 0 ; Current state of the robot, initialized to forward state

; States to indicate direction/location of the robot
forward:	DW 0
forward2:   DW 1
backward: 	DW 2
backward2:  DW 3
...
CTimer_ISR:
	...
	ADDI  -1
	JZERO turn90Sub   
	;; currState can only be between 0 and 5, so it must switch to one of these branches
...
is_count: DW 0      ; Interrupt count, we will perform certain operations after every M_COUNT IS_COUNTS
m_count:  DW 5
...
backwardCall:
	...
	LOAD   is_count  ; Load the Interrupt count
	SUB    m_count   ; Subtract the M_count, which is the maximum interrupt count before calling HugWall subroutine
	JNEG   skip-hugR  ; If IS_COUNT != M_COUNT, we skip hug_wall function :(
	
	STORE  is_count  ; IS_COUNT = 0
	CALL   HugWall
	
    skip-hugR:
    LOAD   is_count
    ADDI   1         
    STORE  is_count  ; IS_COUNT++
    RETURN
...
forwardCall:
	...
	LOAD   is_count  ; Load the Interrupt count
	SUB    m_count   ; Subtract the M_count, which is the maximum interrupt count before calling HugWall subroutine
	JNEG   skip-hug  ; If IS_COUNT != M_COUNT, we skip hug_wall function :(
...
; Takes measurement from sonar 5
; Use e to make an angle correction in the robot
direction:		DW 1				; = 1 when moving forward, = -1 when moving backward
closeThres:  	DW CLOSE_THRES1
farThres:   	DW FAR_THRES1
deltAngle:		DW DELT_ANGLE

HugWall:

	IN 	  DIST5
	SUB   closeThres ; Subtract closest desired distance to wall from sonar value: e = s - c
	OUT   LCD
 	JNEG  MoveAway
	...
	MoveAway:
		...
		JNEG subAngle
		 	...
		 	addAngle:
			LOAD DTheta
			ADD	deltAngle  ; TURN AWAY FROM THE WALL
			STORE DTheta
		JUMP endCase
...
DELT_ANGLE: EQU 5   ; We want to keep 0.5ft = 152.4mm away from the wall
FAR_THRES1:  EQU 838 ; 2.75ft = 838mm
CLOSE_THRES1: EQU 685 ; 2.25ft = 685.8mm
FAR_THRES2:  EQU 304  ; 1ft = 304.8mm
CLOSE_THRES2: EQU 152  ; 0.5ft = 152.4mm
